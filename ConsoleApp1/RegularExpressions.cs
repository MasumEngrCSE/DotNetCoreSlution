...
class RegularExpressions
{
    ...
    private void stringOnly()
    {
        //string pat = @"[a-z|A-Z|\W]";
        string pat = @"[a-z|A-Z]";
        string valCheck = "a-bC-dEf=ghlj!!";
        string ltr = "", nltr = "", tr = "";
        var nonLetter = new List<string>();
        ...
        for (int i = 0; i < valCheck.Length; i++)
        {
            ...
            //tcon = itm + tcon;
            bool ism = Regex.IsMatch(itm.ToString(), pat, RegexOptions.IgnoreCase);
            if (ism)
            {
                ltr = itm + ltr;
            }
